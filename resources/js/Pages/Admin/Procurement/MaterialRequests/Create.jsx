...
export default function Create({ materials, suppliers, categories, prefill = {} }) {
    ...
    const isReplacement = data.type === 'replacement' && prefill.reference_po;

    return (
        <AdminLayout>
            <Head title={isReplacement ? 'Create Replacement Request' : 'Create Material Request'} />
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                                <div>
                                    <h1 className="text-2xl font-bold text-gray-900 tracking-tight">
                                        {isReplacement ? 'Create Replacement Request' : 'Create Material Request'}
                                    </h1>
                                    <p className="mt-1 text-sm text-gray-500">
                                        {isReplacement ? (
                                            <span className="text-amber-600">
                                                ⚠️ This request is for replacement of damaged items from PO #{prefill.reference_po}.
                                            </span>
                                        ) : (
                                            'Request materials for procurement'
                                        )}
                                    </p>
                                </div>
                                <Link
