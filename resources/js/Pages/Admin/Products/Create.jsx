...
export default function Create({ categories, materials, finishMaterials }) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        materials: [],
        parts: [],
        // finishes: [] // REMOVED – unified into materials
    });
    ...
    // --- Materials (unified BOM + finishes) ---
  const addMaterial = () => {
    setData('materials', [
        ...data.materials,
        {
    ...
    const handleSubmit = (e) => {
        ...
        if (data.is_customizable && data.parts.length === 0) {
            alert('Please add at least one furniture part for a customizable product.');
            return;
        }
        if (data.is_customizable) {
            const invalidPart = data.parts.find(p => !p.name || p.dimension_fields.length === 0);
            if (invalidPart) {
                alert('Each part must have a name and at least one dimension field.');
                return;
            }
        }
                    ...
                    <div className="bg-white overflow-hidden rounded-xl shadow-sm border border-gray-100/50">
                        <div className="p-6">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-8">
                                ...
                                {errors.error && (
                                    <div className="rounded-lg bg-red-50 border border-red-200 text-red-800 px-4 py-3 flex items-start shadow-sm">
                                        <div className="flex-shrink-0 mt-0.5">
                                            <svg className="h-5 w-5 text-red-400" fill="currentColor" viewBox="0 0 20 20">
                                                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
                                            </svg>
                                        </div>
                                        <div className="ml-3 text-sm font-medium whitespace-pre-line">{errors.error}</div>
                                    </div>
                                )}
                                ...
                                {/* ====== UNIFIED MATERIALS (BOM + FINISHES) ====== */}
                                <div>
